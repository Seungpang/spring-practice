...
import static me.seungpang.querydsl.entity.QMember.member;
import static me.seungpang.querydsl.entity.QTeam.team;
...
import me.seungpang.querydsl.dto.MemberDto;
import me.seungpang.querydsl.dto.QMemberDto;
import me.seungpang.querydsl.dto.UserDto;
...
class QuerydslBasicTest {
    ...
    void startQuerydsl() { //컴파일 시점에서 오류를 알 수 있다.
        final Member findMember = queryFactory
                .select(member)
                .from(member)
                .where(member.username.eq("member1")) //preparestatement의 파라미터 바인딩 방식을 사용
                .fetchOne();
...
//                .fetchFirst();
    ...
    /**
     * 회원 정렬 순1 1. 회원 나이 내림차순(desc) 2. 회원 이름 오름차순(asc) 단 2에서 회원 이름이 없으면 마지막에 출력(nulls last)
     */
    @Test
    void sort() {
    ...
    void findDtoByJPQL() {
        final List<MemberDto> result = em.createQuery(
                        "select new me.seungpang.querydsl.dto.MemberDto(m.username, m.age) from Member m",
                        MemberDto.class)
                .getResultList();
